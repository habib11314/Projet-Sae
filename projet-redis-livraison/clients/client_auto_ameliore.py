"""
Client Automatique Amélioré (moved to clients/)
Loads restaurants/menu from ../data and generates realistic orders.
"""
...
def charger_restaurants_json():
    """Charge restaurants/menu from ../data to match new structure."""
    try:
        base = os.path.dirname(__file__)
        project_root = os.path.abspath(os.path.join(base, '..'))
        # prefer data/ but fall back to repo root
        possible_restos = [
            os.path.join(project_root, 'data', 'restaurants.json'),
            os.path.join(project_root, 'restaurants.json')
        ]
        possible_menus = [
        ...
        if not chemin_restaurants:
            raise FileNotFoundError(f"restaurants.json introuvable dans: {possible_restos}")
